...
public class GroupController {
     ...
     * 分页查询
     *
     * @param groupPageVO 输入分页信息,limit、page、groupName、agenCode
     *               groupName为空时普通查询，groupName不为空时模糊查询
     * @return limit、page、total、items
     */
    @ApiOperation(value = "分页查询", notes = "输入分页信息,limit、page、groupName、agenCode，" +
            "groupName为空时普通查询，groupName不为空时模糊查询")
    @PostMapping("/listByPage")
    public ResponseResult listByPage(@RequestBody GroupPageVO<GroupVO> groupPageVO) {
        return groupService.listByPage(groupPageVO);
    }
    ...
    @PostMapping("/getGroupByCode")
    public ResponseResult getGroupByCode(@RequestBody GroupPageVO<Object> groupPageVO) {

        return groupService.getGroupByCode(groupPageVO);
    }
}
